...
const queryRoomInfoByRoomId = async (roomId) => {
  try {
    const data = await pool.query('SELECT (id, roomname, roomaddress, hostid, totalNumReviews, totalAccuracy, totalCommunication, totalCleanliness, totalLocation, totalCheckIn, totalValue) FROM rooms WHERE id = $1;', [roomId]);
...
const queryReviewsByRoomId = async (queryObj) => {
  ...
  queryStr += 'date DESC;';
  try {
    const data = await pool.query(queryStr, [queryObj.roomId]);
...
const addReview = async (queryObj) => {
  ...
  try {
    await pool.query(queryStr, [queryObj.userId, queryObj.roomId, queryObj.text, queryObj.date, queryObj.accuracy, queryObj.communication, queryObj.cleanliness, queryObj.location, queryObj.checkIn, queryObj.value]);
  } catch (err) {
...
const updateReview = async (queryObj) => {
  ...
  try {
    await pool.query(queryStr, [queryObj.text, queryObj.accuracy, queryObj.communication, queryObj.cleanliness, queryObj.location, queryObj.checkIn, queryObj.value, queryObj.reviewId]);
  } catch (err) {
...
const deleteReview = async (queryObj) => {
  ...
  try {
    await pool.query(queryStr, [queryObj.reviewId]);
  } catch (err) {
